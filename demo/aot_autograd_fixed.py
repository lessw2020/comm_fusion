...
def create_joint_forward_backward(fn):
    def joint_forward_backward(
        ...
        if not grad_primals:
            print(f"AOT: No grad primals detected!")
        if grad_primals:
...
def call_func_with_args(f, args, steal_args=False):
    print(f"AOT: call func with args - f = {f}")
    if not steal_args:
        args = list(args)
...
def aot_dispatch_autograd(flat_fn, flat_args: List[Tensor], aot_config: AOTConfig):
    ...
    print(f"\n--> AOT: joint fwd_bwd = {joint_forward_backward}\n")
    ...
    with torch.no_grad():
        ...
        with track_graph_compiling("backward"):
            compiled_bw_func = aot_config.bw_compiler(bw_module, deduped_flat_args)
    ...
    class CompiledFunction(torch.autograd.Function):
        ...
        def forward(ctx, *deduped_flat_tensor_args):
            print(f" AOT CClass FORWARD called\n")
        ...
        def backward(ctx, *flat_args):
            print(f"AOT CClass Backward called \n ")
            contiguous_args = [t.contiguous() for t in flat_args]
            all_args = list(ctx.saved_tensors) + list(contiguous_args)
            if CompiledFunction.compiled_bw is None:
                print(f"AOT CC entering None branch")
...
def create_aot_dispatcher_function(
    ...
    with preserve_rng_state(), cross_ref, fake_mode, python_dispatcher_mode:
        ...
        def process_inputs(flat_args):
            print(f"AOT: process_inputs ++++++ ")
            if config.use_fake_tensor:
                print(f"AOT: using FAKE tensors +++++")
                def convert(x):
        ...
        print(f"\nAOT Needs Autograd  ; {needs_autograd} +++++++++++++++++++++\n")
        if needs_autograd:
...
def aot_function(
    ...
    if bw_compiler is None:
        print(f"AOT Problem - fwd is bwd +++++++++++++++++\n")
    ...
    print(f"AOT function: bw compile = {bw_compiler}")
    ...
    def returned_function(*args, **kwargs):
        ...
        if cached_res is None:
            ...
            print(f"++++ AOT compiled_fn and config {aot_config}")
...
def aot_module_simplified(mod: nn.Module, *top_args, **top_kwargs) -> nn.Module:
    ...
    def aot_function_simplified(
        ...
        if bw_compiler is None:
            print(f"AOT: bw compiler is fwd &&&&&& ")
            bw_compiler = fw_compiler
        aot_config = AOTConfig(
            fw_compiler=fw_compiler,
            bw_compiler=bw_compiler,
